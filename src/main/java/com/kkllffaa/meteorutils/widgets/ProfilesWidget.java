...
public class ProfilesWidget {
	...
	public static void create(GuiTheme theme, WContainer container) {
		...
		if (theme.categoryIcons()) {
			window.beforeHeaderInit = wContainer -> wContainer.add(theme.item(Items.ENDER_CHEST.getDefaultInstance()))
					.pad(2);
		}
	...
	protected static void createprofiles(WTable table, GuiTheme theme) {
		...
		for (Profile profile : Profiles.get()) {
			//name and tooltip
			table.add(theme.label(profile.name.get())).expandCellX().widget().tooltip = MyUtils.printif("hud ",
					profile.hud.get()) +
