...
public class ZipUtils {
	...
	public static void adddirectorytozip(File directury, ZipOutputStream stream, ZipEntry entry, int recursive) {
		if (!directury.exists() || !directury.isDirectory() || !directury.canRead())
			return;

		try {

			ZipEntry zipEntry = entry != null
					? new ZipEntry(entry + directury.getName() + "/")
					: new ZipEntry(directury.getName() + "/");
			stream.putNextEntry(zipEntry);
	...
	public static void addfiletozip(File file, ZipOutputStream stream) {
		addfiletozip(file, stream, null);
	}

	/**
	 * @param recursive -1 for infinite
	 */
	public static void adddirectorytozip(File directury, ZipOutputStream stream, int recursive) {
		adddirectorytozip(directury, stream, null, recursive);
	}
