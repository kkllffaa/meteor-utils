...
import com.kkllffaa.meteor_utils.utils.ImmediateBlockIterator;
import com.kkllffaa.meteor_utils.utils.MyInvUtils;
import com.kkllffaa.meteor_utils.utils.MyRenderUtils;
		...
		switch (stage) {
				...
				for (Entity entity : mc.world.getEntities()) {
					...
					if (pistons.stream().anyMatch(s -> entity.getBlockPos().equals(s.offset(mc.world.getBlockState(s).get(Properties.FACING))))) {
						if (((ItemFrameEntity) entity).getHeldItemStack().isEmpty()) {
							...
							if (duperedstonewhenno.get() && (InvUtils.find(Items.REDSTONE).getCount() <= redstonedupeamount.get() || (redstonedupe && InvUtils.find(Items.REDSTONE).getCount() <= 60))) {
								redstonedupe = true;
			...
			case Waitforshulker -> {
				...
				if (currentframe == null || !currentframe.isAlive()) {
					...
					if (redstonedupe && InvUtils.find(Items.REDSTONE).getCount() <= 3) {
						stage = Stage.Activate;
					}else {
						stage = Stage.Placeshulker;
					}
